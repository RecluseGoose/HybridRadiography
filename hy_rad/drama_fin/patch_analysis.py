...
def defectAnalysis(
    ...
    reconKwargs = dict(centre=-1.75, bhc = 0.4, streakCorrection = [0.0,0.4,5.0])   
    ...
    print("nansum0",np.isnan(vol0).sum())
    ...
    for i,filename in enumerate(files):
        ...
        #set edges to zero... basically, the gaussian blur of rois leaves artefacts around edges.
        mask = np.ones_like(diffs2, dtype = np.bool)
        clip = int(1.0 + 2*blur)
        mask[:,clip:-clip,clip:-clip] = False
        diffs2[mask] = 0
        
        #diffs2 = np.array([gaussian_filter(d,postBlur) for d in diffs])
        t = 5
        print("nansum1",np.isnan(diffs2).sum())
    ...
    #    try:
        conts.append([inds, vol1, vol2, diffs2])
    ...
    return conts if (not returnFullVox) else (conts, outvol)

def showDefects(conts, d, show = True):
    plt.close('all')
...
#        ax[2].matshow(diffs[:,50], cmap = 'gray')
        plt.savefig(os.path.join(d,'img_{:03}.png'.format(i)))
    if show:
        plt.show()